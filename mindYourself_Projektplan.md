...
### Akku-Optimierungen (implementiert)

| Bereich | Maßnahme | Effekt |
| --- | --- | --- |
| `HcSyncWorker` (Phone) | WorkManager-Constraints: `requiresBatteryNotLow` + `CONNECTED`; Interval 30 → 60 Min | Kein Sync bei niedrigem Akku; halbe Sync-Häufigkeit |
| `ReminderEvaluationWorker` (Wear) | Frühzeitiger Abbruch außerhalb aktiver Zeitfenster aller Reminder | Spart nachts/tagsüber alle DB-Queries und Location-Checks |
| Wear Data Layer | `.setUrgent()` nur für Log-Daten (Hydration, Supplement); Config-Pushes ohne Urgent | Kein sofortiges Bluetooth-Aufwecken bei Konfigurationsänderungen |
| HealthCache (Wear) | Tägliches automatisches Löschen von Einträgen älter als 7 Tage | Konstante DB-Größe, stabile Query-Performance |
| `PassiveDataService` (Wear) | Herzfrequenz-Writes gebündelt (Buffer: 5 Einträge oder 60 s) | Reduziert DB-Transaktionen von ~60/h auf ~5/h |
...
- **Akku-Effizienz**: Manueller Sync statt automatisch, passive Sensor-Nutzung bevorzugt. Hintergrundprozesse (PassiveListenerService, WorkManager) sind funktional notwendig und davon ausgenommen — Nutzerkontrolle gilt explizit für Sync und manuelle Aktionen. Konkrete Maßnahmen: WorkManager-Constraints für periodische Worker, zeitfensterbasierter Frühabbruch im ReminderEvaluationWorker, selektives `.setUrgent()` nur für zeitkritische Log-Daten, automatische HealthCache-Bereinigung, gebündelte Sensor-Writes.
